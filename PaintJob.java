 ...
 * calculate how many buckets of paint he needs to buy before going to work. 
 * Bob might also have some extra buckets at home. He also knows the area that he can cover 
 * with one bucket of paint.

Write a method named getBucketCount with 4 parameters. The first parameter should be named 
width of type double. This parameter represents the width of the wall.
The second parameter should be named height of type double. This parameter represents the 
height of the wall.
The third parameter should be named areaPerBucket. This parameter represents the area that can be 
covered with one bucket of paint.
The fourth parameter should be named extraBuckets. This parameter represents the bucket count 
that Bob has at home.
...
If one of the parameters width, height or areaPerBucket is less or equal to 0, the method needs
...
return -1to indicate an invalid value.
...
public class PaintJob {
	...
	/*public static int getBucketCount(double width, double height, double areaPerBucket, double extraBuckets) {
		...
		areaPerBucket = width * height;
		return areaPerBucket;
	}
